...
class PlanElementRepositoryTest {
    ...
    fun `When add called expect one row returned`(){
        val result = planElementRepository.add(PlanElementDao(0, Timestamp(1111),1,1,0))
    ...
    fun `When delete called on existing id expect true returned`() {
        planElementRepository.add(PlanElementDao(0, Timestamp(1111),1,1,0))
    ...
    fun `When update called on existing PlanElementDao expect true returned`(){
        val result = planElementRepository.update(PlanElementDao(1, Timestamp(1111),1,1,0))
        Assert.assertTrue(result)
    }

    @Test
    fun `When update called on non existing PlanElementDao expect false returned`(){
        val result = planElementRepository.update(PlanElementDao(0, Timestamp(1111),1,1,0))
        Assert.assertTrue(!result)
    }
}
